...
class Maskdto:
    ...
    def _validate_masks(self) -> None:
        ...
        for name, mask in self.mask_items:
            ...
            if mask.dtype != torch.bool:
                raise ValueError(f"{name} must be boolean tensor, got dtype {mask.dtype}")
    ...
    def all_masks(self):
        ...
        return [self.strong_pos_mask, self.weak_pos_mask, self.pos_mask, 
                self.neg_mask, self.eye_mask]
    ...
    # ----------- Utility Methods -----------
    def apply_eye_mask(self, tensor: torch.Tensor, fill_value: float = 0.0) -> torch.Tensor:
    ...
    def __repr__(self) -> str:
        ...
        return (f"MaskDTO(shape={self.shape}, device={self.device}, "
