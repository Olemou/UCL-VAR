...
class ThermalAugmentor:
    ...
    def thermal_erase(self, img,  
        ...
        for _ in range(max_attempts):
            ...
            if not (x1 + mask_w > center_x1 and x1 < center_x2 and
                    y1 + mask_h > center_y1 and y1 < center_y2):
                x[y1:y1+mask_h, x1:x1+mask_w, :] = 0
                break
        x = np.clip(x, 0, 255).astype(np.uint8)
        return Image.fromarray(x[:, :, 0], mode='L') if x.shape[2] == 1 else Image.fromarray(x)
    ...
    def thermal_contrast(self, x: Image.Image, alpha: float | None = None) -> Image.Image:

        if alpha is not None and alpha > 1:
            raise ValueError(f"alpha must be <= 1 to increase contrast, got {alpha}")
        # Determine the contrast scaling factor
        factor = np.random.uniform(0.5, 1.0) if alpha is None else np.random.uniform(alpha, 1 + alpha)
    ...
    def elastic_transform(
            ...
            distorted_channels = [
                map_coordinates(arr[:, :, i], indices, order=1, mode='reflect').reshape(h, w)
                for i in range(c)
            ]
...
def thermal_erase(img: Image.Image,
                   **kwargs
                  ) -> Image.Image:
    """
...
def thermal_contrast(img: Image.Image,
                     **kwargs
                      ) -> Image.Image:
    """
