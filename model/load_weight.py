...
def load_pretrained_vit_weights(
    custom_model, device, model_size="base", patch_size=16, img_size=224
):
    """
    ...
    if device is None:
        raise ValueError(
            "Device must be specified (e.g., torch.device('cuda') or torch.device('cpu'))"
        )
    ...
    try:
        pretrained_model = timm.create_model(timm_model_name, pretrained=True).to(
            device
        )
    except Exception as e:
        raise ValueError(
            f"❌ Unsupported ViT model '{timm_model_name}'. "
